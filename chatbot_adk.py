"""
ClaimJet Chatbot UI - ADK Version
...
def chat_with_agent(message: str, history: list) -> str:
    """
    Process user message through ADK agent

    Args:
        message: User's input message
        history: Conversation history (not used yet, but available for future)
...
def create_ui():
    ...
    with gr.Blocks(
        ...
        gr.Markdown(
            """
            # ✈️ ClaimJet - EU261 Flight Compensation Assistant (ADK)
        ...
        gr.Markdown(
            """
            ---
            **Powered by Google Gemini 2.5 Flash with ADK**
